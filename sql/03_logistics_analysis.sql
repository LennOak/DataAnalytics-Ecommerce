...
WITH tb_order_delivery_days AS (
SELECT 
    c.customer_state,
	o.order_id,
    DATEDIFF(day, o.order_purchase_timestamp, o.order_delivered_customer_date) AS lead_time_days,
	DATEDIFF(day, o.order_purchase_timestamp, o.order_delivered_carrier_date) AS dispatch_time_days,
...
SELECT
	...
	ROUND(AVG(dispatch_time_days), 2) AS avg_dispatch_time_days,
	ROUND(AVG(shipping_time_days), 2) AS avg_shipping_time_days,
	SUM(is_delayed) AS total_delayed_orders,
	ROUND((SUM(is_delayed * 1.0) / NULLIF(COUNT(order_id), 0)) * 100, 2) AS delay_rate_percentage
FROM tb_order_delivery_days
...
SELECT 
    c.customer_state,
    ROUND(AVG(DATEDIFF(day, o.order_delivered_customer_date, o.order_estimated_delivery_date)), 2) AS avg_days_ahead_of_estimate
FROM olist_orders_dataset o
INNER JOIN olist_customers_dataset c 
	ON o.customer_id = c.customer_id
WHERE o.order_status = 'delivered'
